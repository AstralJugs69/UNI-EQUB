...
| P2-609 | Agent | Add audit writes to auth, KYC, group formation, contribution attempts, draw, payout, restrictions, group freeze, and report export. | In Progress | P2-204 | Sensitive actions produce audit events with actor/entity metadata. | KYC submission/decision audit writes exist in `supabase/functions/kyc-submit-review/index.ts`; group freeze/create/resolve audit writes exist in `supabase/functions/_shared/groupFreeze.ts`; broader workflow audit coverage remains pending |
...
| ID | Owner | Task | Status | Depends On | Acceptance Condition | Required Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| P2-801 | Agent | Update navigation routes for formation, requests, invitations, payout reserve, reliability, audit/admin views. | In Progress | Relevant backend services | Navigation supports Phase 2 screens without breaking MVP routes. | Formation detail, creator, join-with-code, group status ring, and in-app demo routes exist in `mobile/src/navigation/routes.ts` and `mobile/src/navigation/AppNavigator.tsx`; reliability/audit-specific views still pending |
| P2-802 | Agent | Add loading/empty/error/retry states for all new Phase 2 screens. | In Progress | P2-801 | No new screen relies on blank/error-prone states. | Formation screens, join-with-code screen, `DemoTourScreen`, redesigned Explore empty states, and group-status ring loading/frozen/empty winner-history states include handling where currently applicable; final screenshot evidence still pending |
...
| P2-804 | Agent | Add admin dashboard sections for formation queue, KYC queue, payment attempts, defaults, restrictions, audit, and reports. | In Progress | Backend services | Admin can operate Phase 2 without raw database access. | Admin formation/KYC queues and frozen-group manual recovery actions exist; payment-attempt/default/restriction/audit sections and screenshots remain pending |
...
| ID | Owner | Task | Status | Depends On | Acceptance Condition | Required Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| P2-901 | Agent | Expand Jest/service tests for Phase 2 helper logic and critical flows. | In Progress | Implemented phases | Tests cover formation, obligations, attempts, duplicate callbacks, payout maturity, restrictions, notifications, audit. | `mobile/src/services/mock/mockBackend.test.ts` covers direct login failure cases, signup OTP retention, public/private invite-code lookup/redeem/reuse/idempotency, five-member readiness blocking, formation, admin approval, legacy compatibility, auto-draw, USSD, and seeded demo reset; deeper Edge helper behavior tests still pending |
| P2-902 | Agent | Add validation scripts for Phase 2 backend scenarios. | In Progress | Implemented phases | Scripts can generate evidence JSON for key scenarios. | `mobile/scripts/validate-direct-login.js`; `mobile/scripts/validate-phase2-*.js`; `mobile/scripts/validate-phase2-in-app-demo.js`; `mobile/scripts/validate-phase2-member-ui-polish.js`; `mobile/scripts/validate-phase2-durable-notifications.js`; evidence JSON files under `Build/delivery/evidence/`; direct-login/shared/mobile-formation validators check structured Edge/live error extraction, member UI polish validator checks dashboard/explore/group-status contributor-ring contracts, and durable-notifications validator checks inbox/read-state migration; final screenshot/UAT scripts still pending |
