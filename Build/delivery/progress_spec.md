...
Current Wave: Stage C live auth/KYC is active, Stage D live group lifecycle is now active, and Stage B mock payment parity is being hardened around the USSD contribution flow  
...
- Still pending for real MVP completion:
  - final env file wiring instead of placeholder client config
  - live payment reconciliation path
  - reminder derivation and notification replacement
...
| 06 | A6 | Stage A | Establish environment/config strategy for mobile and backend | Plan Stage A | In Progress | A1 | placeholder Supabase client only | Explicit env file strategy documented and wired | Still using placeholder Supabase client values |
...
| 26 | C5 | Stage C | Implement real KYC file storage and image reference handling | Plan Stage C | In Progress | A6, C1 | `supabase/functions/kyc-submit-review/index.ts` | `Student_ID_Img` stores real uploaded reference | KYC submit backend is live, real file upload path still placeholder |
...
| 36 | E3 | Stage E | Implement sender phone normalization in backend | Plan Stage E | Not Started | E2 | None | `+251` normalization and matching logic exists live | Pending |
...
| 39 | E6 | Stage E | Add Chapa sandbox adapter behind payment contract | Plan Stage E | Not Started | E1 | None | Chapa path exists without changing app service contract | Pending |
...
| 43 | F4 | Stage F | Implement live payout wallet-clearance mutation | Plan Stage F | In Progress | F3 | `livePaymentsService.ts`, `payout-withdraw/` scaffold, direct internal-clearance path active in app | Pending payout transitions out of the wallet ledger without external gateway dependency | Mobile flow is active, but the dedicated edge-function route still needs final stabilization |
| 44 | F5 | Stage F | Implement live unpaid-member reminder derivation | Plan Stage F | Not Started | F1 | None | Reminder queue derives from memberships minus successful contributions | Pending |
| 45 | F6 | Stage F | Replace wallet/notification/reminder UI with live backend data | Plan Stage F | In Progress | F3-F5 | `livePaymentsService.ts` | Wallet and notifications no longer depend on mock backend | Wallet/history are live; notifications and reminders are still mock-backed |
| 46 | G1 | Stage G | Implement live admin metrics/overview data | Plan Stage G | Not Started | C-D-F stages | None | Admin dashboard uses real backend metrics | Pending |
| 47 | G2 | Stage G | Implement real report export generation (PDF/CSV) | Plan Stage G | Not Started | G1 | None | Export outputs are generated, not placeholder strings | Pending |
...
| 54 | H4 | Stage H | Generate debug APK artifact and document path | Plan Stage H | Not Started | H2 or H3 | None | Debug APK build path recorded in evidence log | Pending |
...
| 56 | H6 | Stage H | Prepare UAT checklist and final delivery evidence pack | Plan Stage H | Not Started | H2-H5 | None | Delivery evidence complete and traceable | Pending |
...
| M2 Auth / KYC | Live auth, OTP, and KYC review path are active; storage upload still partial | In Progress |
...
| M4 Payments | Live simulated contribution writes and live history reads are active; normalization and provider adapters are still partial | In Progress |
| M5 Auto Draw / Payout / Reminders | Live round completion and payout creation are active; withdrawal path is partial and reminders remain pending | In Progress |
| M6 Admin / Reports / Hardening | Admin surfaces and mock reports exist, hardening incomplete | In Progress |
...
## 6. Open Risks
- Live backend implementation is only partial; auth and KYC are persisted through Supabase, group lifecycle is partly live, but payments, rounds, and reports still depend on the mirrored mock layer.
- The dedicated `payout-withdraw` edge route is still inconsistent in the hosted project, so the app currently uses a direct internal-clearance update for the wallet-clearance step.
